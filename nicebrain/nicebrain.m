...
if isempty(mriDICOMfilenames)
    fprintf('No MRI DICOM files found in %s/DICOM/MRI/', subjectDirectory);
    return;
elseif isempty(ctDICOMfilenames) && coreg
    fprintf('No CT DICOM files found in %s/DICOM/CT/', subjectDirectory);
    return;
end
...
shellcommand = [scriptDirectory '/viewMRIandCT.sh ' fsDirectory ' ' subjectDirectory '&'];
system(shellcommand)
...
elecMatrix = sortelec();
save(strcat(electrodesDirectory, '/', 'electrodes.mat'), 'elecMatrix');
