...
%%setup the parpool to work on openmind
%fprintf('setting LocalUseMpiexec to false \n');
...
for k=band_idx
    ...
    %c.JobStorageLocation = t;
    %c.NumWorkers = nWorkers;
    %mypool = parpool(c, nWorkers, 'SpmdEnabled', false);
    parfor idx_channel=1:size(signal,2)
        ...
        %hilbert_transform_output = hilbert_transform(double(transpose(signal(:,idx_channel))),parameters.SamplingRate.NumericValue,filter_bank);
        %signal_hilbert_bp_cell = [];
        %for i=1:length(hilbert_transform_output)
        %    signal_hilbert_bp_cell(i) = abs(hilbert_transform_output(i));
        %end
        % mean across
