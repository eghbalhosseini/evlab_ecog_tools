function [dataout]=filter_channels_MGH(datafile,subject_op_info)
% varargin : 
...
unique_op_tags=unique(op_ch_tags);
ch_names={datafile.ElectrodesInfo.Label};
param=ecog.param;
param.channels=[datafile.ElectrodesInfo.ElectrodeID];
param.channels_ID={datafile.ElectrodesInfo.Label};
dig_loc=regexp(ch_names,'w*\d');
% 
...
ch_num=arrayfun(@(x) ch_names{x}(dig_loc{x}:end),valid_label,'uni',false);
unique_tags=unique(ch_tag);
ecog_tags=intersect(unique_tags,unique_op_tags)';
% 
ecog_loc_for_tag=cellfun(@(x) find(...
    cell2mat(...
    cellfun(@(y) ~isempty(regexp(y,[x,'*\d'])),ch_names,'uni',false))==1)',ecog_tags,'uni',false);
...
decimation_factor = parameters.SamplingRate / ecog.param.samplingrate;
...
signal_hilbert_decimated_loop = zeros(size(decimate(double(signal_hilbert_zs_loop(:,1)),decimation_factor),1),size(signal_hilbert_zs_loop,2),'double');
     %    %
...
dataout.MetaTags.DownSampleFrequency=ecog.param.samplingrate;
param_fields=fields(param);
for k=1:length(param_fields)
    dataout.MetaTags.(param_fields{k})=param.(param_fields{k});
end 
subj_file_name=strcat(dataout.MetaTags.FilePath,'_',...
