function [trial_timing_dec] = get_timing_from_json_files_LangLocAudio_v2(ecog_obj,all_trial_timing,events_table,audio_align_path,with_wavelet,do_downsample)
...
%
if do_downsample
    [P,Q] = rat(ecog_obj.for_preproc.decimation_freq/ecog_obj.sample_freq);
    output_samples=ecog_obj.for_preproc.decimation_freq;
else
    P=1;
    Q=1;
    output_samples=ecog_obj.sample_freq;
end 
...
trial_aud_ts=diff([all_trial_timing{k,2}{1:2}])./ecog_obj.sample_freq;
fprintf('timing difference %f ms\n',(orig_aud_ts-trial_aud_ts)*1000);

    [P_aud,Q_aud] = rat(output_samples/orig_fs);
...
for p=1:size(word_align,1)
    ...
    word_idx_dec=[ceil(word_st_ts*output_samples),floor(word_end_ts*output_samples)];
