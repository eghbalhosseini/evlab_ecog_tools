...
%% method for constructing the object 
        function obj = ecog_analysis(ecog_data)
        ...
        function  overlay_lang_resp_elec_on_brain(obj,varargin)
            ...
            % plot all electrodes 
            L_loc=cellfun(@(x) x(1)=='L',elec_label);
            if any(L_loc)
                left_tbl=elec_loc(L_loc,:);
                for kk=1:size(left_tbl,1)
                    R=left_tbl(kk,:).R; A=left_tbl(kk,:).A; S=left_tbl(kk,:).S;
                    h=plot3(R,A,S);
                    h.Marker='o';h.MarkerFaceColor=[1,.3,.3];h.MarkerEdgeColor=[1,.3,.3];h.MarkerSize=5;
                end 
            end 
            ...
            % plot electrodes on the right side 
                        % plot all electrodes 
            R_loc=cellfun(@(x) x(1)=='R',elec_label);
            if any(R_loc)
                right_tbl=elec_loc(R_loc,:);
        ...
        function plot_channel_resp_with_anatomy(obj,varargin)
            %METHOD1 Summary of this method goes here
            %   Detailed explanation goes here
                        p=inputParser();
            addParameter(p, 'average_brain', false);
...
%                     %
                    ...
                    ax.XAxis.LineWidth=2;
                    ax.YAxis.LineWidth=2;
                    analysis_path=strcat(obj.ecog_data.save_path,'analysis/average_timecourse/');
                        if ~exist(strcat(analysis_path,obj.ecog_data.subject_id))
                            mkdir(strcat(analysis_path,obj.ecog_data.subject_id));
                        end
                        set(gcf,'PaperPosition',[.25 .25 8 6]);
                        set(gcf,'PaperOrientation','landscape');
                        print(f, '-bestfit','-dpdf','-opengl', strcat(analysis_path,obj.ecog_data.subject_id,'/',obj.ecog_data.subject_id,'_lang_electrode_resp_',obj.ecog_data.modality,'_',ops.elec_mode,'_zs_',num2str(ops.zscore),'.pdf'));
                        close(f)
        end 
        
        function  overlay_lang_resp_elec_on_mni_brain(obj,varargin)
            %METHOD1 Summary of this method goes here
            %   Detailed explanation goes here
            p=inputParser();
            addParameter(p, 'average_brain', false);
            ...
            switch ops.elec_mode
                case 'bip_elec'
                    elec_loc=obj.ecog_data.bip_ch_pos_mni;
                    elec_label=obj.ecog_data.bip_ch_label_valid;
                    elec_label=cellfun(@(x,y) erase([x,'-',y],'_'),elec_label(:,1),elec_label(:,2),'uni',false);
                    if ops.zscore
                        elec_flag='bip_elec_data_zs_dec';
                ...
                case 'elec'
                    elec_loc=obj.ecog_data.elec_ch_pos_mni;
                    elec_label=cellfun(@(x) erase(x,'_'),obj.ecog_data.elec_ch_label,'uni',false);
                    if ops.zscore
                        elec_flag='elec_data_zs_dec';
                    else 
            ...
            %assert(all(cellfun(@(x,y) strcmp(x,y),elec_loc.label,elec_label)),'labels dont match!');
            sig_elec_tbl=elec_loc(sig_elec_loc,:);
            sig_elec_label=elec_label(sig_elec_loc,:);
            f=figure(1);
            clf(f)
            f.Units='Inches';
            f.PaperOrientation='landscape';
            ...
            ax=axes('position',[.1,.3,.4,.4*pap_ratio]);
            hold on
            nvertices=size(obj.ecog_data.icmb152_pial.Vertices_lh,1);
            color_data = repmat([255,229,204]./255,nvertices,1);
            %color_data(lh_pial.curve>0,:) = 0.5*[255,229,204]./255;
            patch_handle = patch('vertices', obj.ecog_data.icmb152_pial.Vertices_lh, 'Faces', obj.ecog_data.icmb152_pial.Faces_lh, 'FaceVertexCData', color_data,'FaceLighting','gouraud','SpecularStrength',0,'DiffuseStrength',0.7);
            shading interp;
            xlim(2*50*[-1 1]); ylim(2*50*[-1 1]-20); zlim(2*50*[-1 1]+40);
            %campos(1e3*[-1.5580    0.0302    0.0301])
            view([-90 0]);
            patch_handle.FaceAlpha=.4;
            light_handle = camlight('left','infinite');
            ...
            % plot significant electrodes on the left side 
            L_loc=cellfun(@(x) x(1)=='L',sig_elec_label);
            if any(L_loc)
                left_tbl=sig_elec_tbl(L_loc,:);
                for kk=1:size(left_tbl,1)
                    loc_=left_tbl{kk};
            ...
            ax=axes('position',[.5,.3,.4,.4*pap_ratio]);
            hold on
            nvertices=size(obj.ecog_data.icmb152_pial.Vertices_rh,1);
            color_data = repmat([255,229,204]./255,nvertices,1);
            %color_data(lh_pial.curve>0,:) = 0.5*[255,229,204]./255;
            patch_handle = patch('vertices', obj.ecog_data.icmb152_pial.Vertices_rh, 'Faces', obj.ecog_data.icmb152_pial.Faces_rh, 'FaceVertexCData', color_data,'FaceLighting','gouraud','SpecularStrength',0,'DiffuseStrength',0.7);
            shading interp;
            xlim(2*50*[-1 1]); ylim(2*50*[-1 1]-20); zlim(2*50*[-1 1]+40);
            %campos(1e3*[-1.5580    0.0302    0.0301])
            view([90 0]);
            ...
            % overlay significant electrodes
            R_loc=cellfun(@(x) x(1)=='R',sig_elec_label);
            if any(R_loc)
                right_tbl=sig_elec_tbl(R_loc,:);
                for kk=1:size(right_tbl,1)
                    loc_=right_tbl{kk};
