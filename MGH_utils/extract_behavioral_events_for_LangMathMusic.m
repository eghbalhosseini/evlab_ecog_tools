function [events_table] = extract_behavioral_events_for_LangMathMusic(varargin)
...
for nn=1:size(d_events)
    ...
    tags=event_table.onset*sampling_freq;
    all_events_Table{nn,1}=[event_table];
    condition=event_table.phase;
    non_fixation=find(~strcmp(condition,'F'));
    fname=event_table.fname;
    audio_filename=event_table.fname;
    block=event_table.block;
    audio_offset=event_table.audio_offset*sampling_freq;
    response_offset=event_table.response_offset*sampling_freq;
    trial=event_table.trial;
